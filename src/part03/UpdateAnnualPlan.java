...
public class UpdateAnnualPlan {
    ...
    public UpdateAnnualPlan(ExhibitManagement exhibitManagement, ExhibitionPlan exhibitionPlan) {
        ...
        try {
            ...
            this.HTML += "<script src='http://localhost:9990/part03/updateAnnualPlanFunctions.js'></script>";
            this.HTML += "<div class='flex flex-col justify-around w-11/12 max-h-full'>";
            this.HTML += "<form class='flex flex-col justify-around h-full' action='update_annual_plan/update' method=\"POST\" enctype=\"multipart/form-data\">";
            this.HTML += "<input type=hidden id='hallNoInputHidden' name='hallNoInput' value='"
                    + exhibitionPlan.getNumberOfHalls() + "'>";
            this.HTML += "<div class='overflow-x-auto bg-gray-400 p-4 rounded-lg flex flex-row justify-center'>";
